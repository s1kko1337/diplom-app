  ...
  <Dialog :open="show" @update:open="handleOpenChange">
    <DialogContent class="max-w-lg max-h-[80vh] flex flex-col">
        ...
        <div>
          ...
          <div class="space-y-2">
            <button
              ...
              :key="wt.type"
              class="w-full text-left p-3 border rounded-md transition-all duration-150"
              :class="
        ...
        <div v-if="needsSensor">
          ...
          <div class="grid grid-cols-2 gap-2">
            <button
              v-for="s in sensors"
              :key="s.id"
              class="text-left p-2 border rounded-md transition-all duration-150"
              :class="
                selectedSensor === s.id
                  ? 'border-primary bg-surface-2'
                  : 'border-border hover:bg-surface-2'
      ...
      <DialogFooter class="border-t border-border pt-4 gap-2">
        <button
          class="px-4 py-2 border border-border rounded-md text-sm hover:bg-surface-2 transition-all duration-150"
          @click="handleClose"
        >
          ОТМЕНА
        </button>
        <button
          :disabled="!canAdd"
          class="px-4 py-2 border border-primary rounded-md text-sm hover:bg-surface-2 transition-all duration-150 disabled:opacity-30"
          @click="handleAdd"
        >
          ДОБАВИТЬ
        </button>
